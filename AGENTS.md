...
- Read the pinned private roadmap's common reading list and the selected slice's **Read first**, **Acceptance cases**, and reviewed packet. Work one slice at a time; IDs are not execution order. All hard prerequisites need immutable verified evidence.
- Confirm the packet and checkpoint name the exact writable files. Component descriptions and packet previews are not a writable-file allowlist. Amend scope explicitly before touching another file.
...
- Update the checkpoint at slice start and after material validation. Keep completed-slice evidence. Stop after the requested slice; do not auto-advance.
