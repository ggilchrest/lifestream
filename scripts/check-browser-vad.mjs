...
try{
  ...
  const result=await page.evaluate(async base64=>{
    const {SileroV5}=await import('/control/vad-bundle.js'),{SpeechGate}=await import('/control/speech-gate.js');
    ...
    const cases=[];
    try{for(const [name,samples] of [['silence',silence],['synthetic-key-impulses',impulses],['synthetic-room-noise',room],['normal-synthetic-speech',speech],['quiet-synthetic-speech',speech.map(x=>x*.1)]]){
      model.reset_state();const gate=new SpeechGate(50);let qualified=0,first=null,maximum=0;
      for(let offset=0;offset<samples.length;offset+=512){const frame=new Float32Array(512);frame.set(samples.subarray(offset,offset+512));const {isSpeech}=await model.process(frame);maximum=Math.max(maximum,isSpeech);if(gate.update(isSpeech,32)){qualified++;first??=offset/16;}}
      cases.push({name,qualifiedFrames:qualified,firstQualifiedMs:first,maximumProbability:maximum,durationSeconds:samples.length/16000});
    }}finally{await model.release();}
    return cases;
  },pcm.toString('base64'));
  const record=evidence.startCase('corpus',{cases:result,claim:'specified synthetic fixtures through real browser Silero; excludes coughs, physical AEC and untested rooms'});
