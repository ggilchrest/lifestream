...
try{
  ...
  const result=await page.evaluate(async({base64,noiseFixture})=>{
    const {SileroV5}=await import('/control/vad-bundle.js'),{SpeechGate,PRE_ROLL_FRAMES}=await import('/control/speech-gate.js'),{VadStream}=await import('/control/vad-stream.js');
    ...
    let rejectFrame;const vad=new VadStream(model,()=>{},error=>rejectFrame?.(error));
    const processFrame=frame=>new Promise((resolve,reject)=>{rejectFrame=reject;vad.onFrame=(_frame,p)=>resolve(p);vad.push(frame);});
    ...
    try{for(const [name,samples] of [['silence',silence],['synthetic-key-impulses',impulses],['synthetic-room-noise',room],[noiseFixture?'provided-noise':'normal-synthetic-speech',speech],[noiseFixture?'quiet-provided-noise':'quiet-synthetic-speech',speech.map(x=>x*.1)]]){
      model.reset_state();vad.idleFrames=0;const gate=new SpeechGate(50),bargeGate=new SpeechGate(50);let qualified=0,bargeQualified=0,first=null,maximum=0;
      for(let offset=0;offset<samples.length;offset+=512){const frame=new Float32Array(512);frame.set(samples.subarray(offset,offset+512));const isSpeech=await processFrame(frame);maximum=Math.max(maximum,isSpeech);if(gate.update(isSpeech,32)){qualified++;first??=offset/16;}if(bargeGate.update(isSpeech,32,true))bargeQualified++;}
