...
const insert=db.prepare('INSERT INTO understanding_projection VALUES (?,?,?,?,?,?)');
...
function step(){
 ...
 try{db.exec('BEGIN IMMEDIATE');for(const row of rows){remove.run(row.projection_id);insert.run(row.projection_id,row.artifact_id,row.scope_key,row.boundary,row.content,row.fresh_until_ms);}db.exec('COMMIT');batches++;}
