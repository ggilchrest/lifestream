// Actual rendered S080 journey against the existing selected development provider.
...
const report={slice:'LS-S080',collectedAt:new Date().toISOString(),checks:[],exclusions:['No personal data, physical output, Human fit acceptance, training, deployment, production or provider service changes']};let tunnel,root,key;
try{
 ...
 key=await loadWindowsKey();root=await mkdtemp(join(tmpdir(),'ls-s080-report-'));const output=join(root,'report.json');
 await exec(process.execPath,['--test','apps/control-web/test/relationship-recovery.test.mjs'],{cwd:repository,env:{...process.env,LIFESTREAM_INFERENCE_API_KEY:key,LIFESTREAM_S080_PROVIDER:'selected',LIFESTREAM_S080_REPORT:output},timeout:120000,maxBuffer:1024*1024});
 const evidence=JSON.parse(await readFile(output,'utf8'));assert.equal(evidence.result,'pass');assert.equal(evidence.selectedProvider,true);report.checks.push({id:'LS-TEST-102-104-rendered-selected-provider',result:'pass',evidence});report.result='pass';
}catch(error){report.result='fail';report.blocker=error instanceof assert.AssertionError?error.message:'Bounded rendered/provider check failed; no credentials logged.';report.failureEvidence=typeof error.stdout==='string'?error.stdout.slice(-12000):undefined;process.exitCode=1;}
