...
const dispatchEnabled=process.env.PWCE_HOST_DISPATCH==='1';
...
try{
 ...
 if(dispatchEnabled){
  ...
  const fenced=await prepare(),beforeFence=dispatchPosts;app.pwceJournal.close();const rejected=await request(confirmPath(fenced),confirmation(fenced));assert.ok(rejected.status>=500,JSON.stringify(rejected));assert.equal(dispatchPosts,beforeFence);checks.push('unavailable durable journal prevents any admission or invocation POST');
