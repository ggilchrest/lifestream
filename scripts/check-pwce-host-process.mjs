...
try{
 ...
 const discovered=await request(path);assert.equal(discovered.status,200,JSON.stringify(discovered));assert.equal(discovered.body.tools[0].capabilityId,'pwce.home.light.set-level');assert.equal(discovered.body.tools[0].authorization,'invokeDecision');assert.equal(discovered.body.tools[0].inputSchema.properties.parameters.properties.level.maximum,1);assert.equal(discovered.body.actionAdministration,'providerReview');checks.push('authenticated host discovers actual producer catalog and exact input schema');
 ...
 if(process.env.PLAYWRIGHT_MODULE){
  ...
  try{
   ...
   await page.locator('#cap-refresh').click();await page.waitForFunction(()=>document.querySelector('#cap-select')?.options.length>1);
   await page.locator('#cap-select').selectOption('0');assert.equal(await page.locator('#cap-prepare').isEnabled(),true);
   await page.locator('[data-argument=siteRef]').fill('home.one');await page.locator('[data-argument=targetEntityId]').fill('light.browser');await page.locator('[data-argument=level]').fill('0.3');await page.locator('#cap-prepare').click();await page.getByRole('heading',{name:'2. Review the exact action',exact:true}).waitFor();assert.ok((await page.locator('#cap-pwce-workflow').textContent()).includes('light.browser'));if(dispatchEnabled)assert.equal(await page.getByRole('button',{name:'Run reviewed action',exact:true}).isDisabled(),true);else assert.ok((await page.locator('#cap-pwce-workflow').textContent()).includes('not configured'));checks.push('actual Chrome prepares labeled PWCE arguments and renders exact review without execution');
