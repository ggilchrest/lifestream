...
const report={slice:'LS-S081',collectedAt:new Date().toISOString(),checks:[],exclusions:['No personal data, physical output, Human fit acceptance, training, deployment, production or provider service changes']};let tunnel,root,key;
try{
 ...
 key=await loadWindowsKey();root=await mkdtemp(join(tmpdir(),'ls-s081-report-'));const output=join(root,'report.json');
 await exec(process.execPath,['--test','apps/control-web/test/relationship-isolation-continuation.test.mjs'],{cwd:repository,env:{...process.env,LIFESTREAM_INFERENCE_API_KEY:key,LIFESTREAM_S081_CONTINUATION_PROVIDER:'selected',LIFESTREAM_S081_CONTINUATION_REPORT:output},timeout:420000,maxBuffer:1024*1024});
 const evidence=JSON.parse(await readFile(output,'utf8'));assert.equal(evidence.result,'pass');assert.equal(evidence.selectedProvider,true);report.checks.push({id:'LS-TEST-106-independent-isolation-continuation',result:'pass',evidence});report.result='pass';
