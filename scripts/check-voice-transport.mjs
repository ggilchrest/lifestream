...
try{
  for(const mode of ['original',...(process.env.VOICE_CANCEL_DURING_COMPUTE==='1'?['cancelBeforePcm']:[]),'cancel','recovery']){
    const controller=new AbortController(),id=crypto.randomUUID(),text=mode.startsWith('cancel')?corpus.long[0]:corpus.original;
    ...
    const cancelTimer=mode==='cancelBeforePcm'?setTimeout(()=>{cancelAt=performance.now();controller.abort();},2000):null;
    try{for await(const event of synthesizePreview(provider,input,controller.signal)){
    ...
    assert.equal(terminal?.outcome,mode.startsWith('cancel')?'cancelled':'succeeded',JSON.stringify(terminal));
    if(mode==='cancelBeforePcm')assert.equal(frames,0,'This case must interrupt sentence-batch compute before decoded PCM');
