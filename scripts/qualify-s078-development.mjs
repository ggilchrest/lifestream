// Actual rendered S078 journey against the existing selected development provider.
...
const report={slice:'LS-S078',collectedAt:new Date().toISOString(),checks:[],exclusions:['No personal data, physical output, Human fit acceptance, training, deployment, production or provider service changes']};let tunnel,root,key;
try{
 ...
 key=await loadWindowsKey();root=await mkdtemp(join(tmpdir(),'ls-s078-report-'));const output=join(root,'report.json');
 await exec(process.execPath,['--test','apps/control-web/test/relationship-lab.test.mjs'],{cwd:repository,env:{...process.env,LIFESTREAM_INFERENCE_API_KEY:key,LIFESTREAM_S078_PROVIDER:'selected',LIFESTREAM_S078_REPORT:output},timeout:320000,maxBuffer:1024*1024});
 const evidence=JSON.parse(await readFile(output,'utf8'));assert.equal(evidence.result,'pass');assert.equal(evidence.selectedProvider,true);report.checks.push({id:'LS-TEST-099-rendered-selected-provider',result:'pass',evidence});report.result='pass';
