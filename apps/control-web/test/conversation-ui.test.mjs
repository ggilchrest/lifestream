...
test('browser-first conversation surface keeps physical IO in the browser', async () => {
  ...
  for (const text of ['Runtime profile', 'Mac-local', 'ai5090', 'Apply profile', 'Start voice / Connect', 'Speaker test', 'Assistant TTS test', 'Assistant STT test', 'Stop response', 'Microphone']) assert.match(html, new RegExp(text));
  for (const text of ['getUserMedia', 'isSecureContext', 'getSettings', 'AudioContext', 'AbortController', 'WebSocket', 'commitTurn', 'stopPlayback', 'barge-in speech detected', '/api/runtime/v1/profile', '/api/runtime/v1/messages', '/api/runtime/v1/tts', '/api/runtime/v1/stt', 'createBufferSource', 'resample16k']) assert.match(js, new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
