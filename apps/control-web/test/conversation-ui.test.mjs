...
test('browser-first conversation surface keeps physical IO in the browser', async () => {
  ...
  assert.doesNotMatch(js, /SGLANG_API_KEY|30000|ai5090|v1\/chat\/completions/); assert.match(js, /video:false/); assert.match(js, /Retry backend connection/); assert.match(js, /fetch\("\/health"/);
});
