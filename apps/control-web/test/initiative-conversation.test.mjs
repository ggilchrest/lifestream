...
test('output matcher accepts either connection order and rejects stopped, foreign or malformed playback',async t=>{
 ...
 const nextTrace=randomUUID();output.expect();const accepted=output.accept({...delivery,interactionId:nextTrace});output.receive({type:'response',event:{interactionTraceId:nextTrace,payload:{type:'textDelta',text:delivery.text}}});await accepted;output.receive({type:'audio',interactionTraceId:nextTrace,chunk:{frame:frame()}});output.receive({type:'response',event:{interactionTraceId:nextTrace,payload:{type:'terminal',state:'completed'}}});output.stop();output.close();
 let replyCompleted=0;const ordinary=new ConversationOutput({onStopped:()=>{},onState:()=>{},onComplete:()=>assert.fail('An ordinary reply is not an Initiative receipt'),onReplyComplete:()=>replyCompleted++});await ordinary.connect();const replyTrace=randomUUID();ordinary.beginReply(replyTrace);ordinary.receive({type:'response',event:{interactionTraceId:replyTrace,payload:{type:'textDelta',text:'Two speech segments.'}}});
 for(const segmentId of ['first','second'])ordinary.receive({type:'audio',interactionTraceId:replyTrace,chunk:{segmentId,frame:frame()}});ordinary.receive({type:'response',event:{interactionTraceId:replyTrace,payload:{type:'terminal',state:'completed'}}});assert.equal(replyCompleted,0);for(const source of sources.slice(-2))source.dispatchEvent(new Event('ended'));assert.equal(replyCompleted,1);
...
async function syntheticMicrophone(page){
 await page.route('**/vad-stream.js',route=>route.fulfill({contentType:'text/javascript',body:`export class VadStream {static async create(onFrame,onError){const vad=new VadStream();vad.closed=false;window.syntheticVoiceFeed=(count,probability)=>{if(!vad.closed)for(let i=0;i<count;i++)onFrame(new Float32Array(512).fill(.02),probability);};return vad;}push(){}close(){this.closed=true;}}`}));
 await page.evaluate(()=>{window.ort={};window.syntheticMicTracks=[];window.syntheticCaptureCalls=0;navigator.mediaDevices.getUserMedia=async()=>{window.syntheticCaptureCalls++;const audio=new AudioContext(),destination=audio.createMediaStreamDestination();window.syntheticMicAudio=audio;window.syntheticMicTracks.push(...destination.stream.getTracks());return destination.stream;};});
}

test('explicit microphone shares the opening transport, barge-in becomes a scoped voice reply and Stop releases capture',{skip:!process.env.PLAYWRIGHT_MODULE,timeout:60000},async t=>{
 const {chromium}=await import(process.env.PLAYWRIGHT_MODULE),browser=await chromium.launch({channel:'chrome',headless:true});t.after(()=>browser.close());const f=await fixture(t,browser);await syntheticMicrophone(f.page);let recognized=0,capturedFrames=0;
 f.app.providers.stt={async *transcribe(_context,audio){recognized++;for await(const message of audio)if(message.type==='frame')capturedFrames++;yield {kind:'data',payload:{type:'committed',text:'A synthetic spoken reply.'}};yield {kind:'terminal',outcome:'succeeded'};}};
 let connections=0;f.page.on('websocket',()=>connections++);await f.page.locator('#room-modality').selectOption('speech');await f.page.locator('#room-mic-start').click();await f.page.locator('#room-mic-status').filter({hasText:'Microphone on'}).waitFor();assert.equal(await f.page.evaluate(()=>window.syntheticCaptureCalls),1);assert.equal(connections,1);
 await f.page.locator('#room-enable').click();await f.page.locator('#room-output-state').filter({hasText:'Speech ready · microphone on'}).waitFor();await f.page.locator('.room-simulation summary').click();await f.page.locator('#room-source').fill(f.sourceEventId);await f.page.locator('#room-run').click();await f.page.locator('.room-turn-state').filter({hasText:'Endpoint accepted · playback pending'}).waitFor();
 await f.page.evaluate(()=>{window.syntheticVoiceFeed(5,.95);window.syntheticVoiceFeed(25,.05);});await f.page.locator('.room-turn-state').filter({hasText:'Voice playback finished locally'}).waitFor();
 assert.match(await f.page.locator('#room-mic-status').textContent(),/Microphone on/);assert.match(await f.page.locator('#room-status').textContent(),/Voice reply completed/);assert.equal(recognized,1);assert.ok(capturedFrames>0);assert.equal(f.counts().inferenceCalls,2);assert.equal(connections,1);assert.equal(await f.page.locator('.room-turn.user').count(),1);assert.equal(await f.page.locator('.room-turn.assistant').count(),2);assert.equal(f.acks.some(a=>a.kind==='playbackCompleted'),false);assert.equal(f.requests[1].scope.assistantId,f.seed.assistantId);assert.equal(f.requests[1].scope.sessionId,f.seed.sessionId);assert.match(f.requests[1].sections.find(s=>s.kind==='conversation').content,/A synthetic invitation/);
 await f.page.locator('#room-refresh').click();await f.page.waitForFunction(()=>document.querySelector('#room-outcome').textContent.includes('Replied'));assert.match(await f.page.locator('#room-outcome').textContent(),/Cancelled/);
 await f.page.evaluate(()=>window.scrollTo(0,0));await f.page.screenshot({path:'/private/tmp/lifestream-microphone-desktop.png',fullPage:true});await f.page.setViewportSize({width:390,height:844});await f.page.evaluate(()=>window.scrollTo(0,0));await f.page.screenshot({path:'/private/tmp/lifestream-microphone-mobile.png',fullPage:true});assert.ok(await f.page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await f.page.locator('#room-mic-stop').click();await f.page.locator('#room-mic-status').filter({hasText:'Microphone off'}).waitFor();assert.equal(await f.page.evaluate(()=>window.syntheticMicTracks.every(t=>t.readyState==='ended')),true);assert.deepEqual(f.errors,[]);
});

test('Stop during pending microphone permission cannot reopen capture after the permission promise resolves',{skip:!process.env.PLAYWRIGHT_MODULE,timeout:45000},async t=>{
 const {chromium}=await import(process.env.PLAYWRIGHT_MODULE),browser=await chromium.launch({channel:'chrome',headless:true});t.after(()=>browser.close());const f=await fixture(t,browser);
 await f.page.evaluate(()=>{window.syntheticStopped=0;navigator.mediaDevices.getUserMedia=()=>new Promise(resolve=>{window.resolveSyntheticPermission=()=>resolve({getTracks:()=>[{stop:()=>window.syntheticStopped++}]});});});
 await f.page.locator('#room-mic-start').click();await f.page.waitForFunction(()=>!!window.resolveSyntheticPermission);await f.page.locator('#room-mic-stop').click();await f.page.evaluate(()=>window.resolveSyntheticPermission());await f.page.waitForFunction(()=>window.syntheticStopped>0);assert.equal(await f.page.locator('#room-mic-status').textContent(),'Microphone off');assert.equal(await f.page.locator('#room-mic-start').isDisabled(),false);assert.equal(f.counts().inferenceCalls,0);assert.deepEqual(f.errors,[]);
});

test('active synthetic microphone releases its track on server disclosure changes, navigation and sign-out',{skip:!process.env.PLAYWRIGHT_MODULE,timeout:60000},async t=>{
 ...
 for(const action of ['disclosure','navigation','signOut']){
  const f=await fixture(t,browser);await syntheticMicrophone(f.page);await f.page.locator('#room-mic-start').click();await f.page.locator('#room-mic-status').filter({hasText:'Microphone on'}).waitFor();
  if(action==='navigation')await f.nav('relationship');
  else await f.page.evaluate(async action=>{const api=window.lifestreamAuth.request;if(action==='signOut')await api('/api/auth/v1/sign-out',{},true);else {const session=await api('/api/runtime/v1/session-context');await api('/api/runtime/v1/session-context',{expectedRevision:session.revision,mode:'audio',audienceScope:'unknown'},true);}},action);
  await f.page.waitForFunction(()=>window.syntheticMicTracks.every(track=>track.readyState==='ended'));assert.equal(await f.page.locator('#room-mic-status').textContent(),'Microphone off',action);assert.equal(f.counts().inferenceCalls,0,action);assert.equal(f.counts().sttCalls,0,action);assert.deepEqual(f.errors,[],action);
 }
});
