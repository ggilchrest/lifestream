...
export function compileDiscoveryCandidates(parent:UnderstandingRecord,boundary:string,now=Date.now()):UnderstandingRecord[] {
 ...
 const items=brief?[...parent.claims as Record<string,unknown>[],...parent.aliasClaims as Record<string,unknown>[]].slice(0,6):[{text:`What about ${topicRefs.join(', ')} appeals to you?`,sourceRefs:[],claimId:null}];
 return items.map(item=>{
  const alternatives=brief?[]:(parent.explanations as Record<string,unknown>[]).map(explanation=>String(explanation.summary));
  const record:UnderstandingRecord={schemaVersion:'1.0.0',recordType:'candidate',assistantId:parent.assistantId,userId:parent.userId,relationshipId:parent.relationshipId,deploymentId:parent.deploymentId,candidateId:randomUUID(),revision:1,kind:brief?'discovery':'question',status:'proposed',content:String(item.text),topicRefs,groundingRefs:[parentRef,...(item.claimId?[`claim:${item.claimId}`]:[]),...item.sourceRefs as string[]].slice(0,32),hypothesisRefs:brief?[]:[parentRef],alternatives,limitations:brief?['Attributed source assertion, not independently verified consensus.','Optional context: no duty to mention, recommendation, permission or personal preference is established.']:['The underlying explanations remain tentative; asking is optional.',String(parent.unknownAlternative),String(parent.uncertainty)],contextRef:`snapshot:${boundary}`,builtAt:new Date(now).toISOString(),expiresAt:new Date(expires).toISOString(),configurationRef:parent.configurationRef,dependencyRefs:[parentRef,...parent.dependencyRefs as string[]].slice(0,32),scores:{interestStrength:null,evidenceConfidence:null,sourceCoverage:null,knowledgeCoverage:null,knowledgeReliability:null,expectedUsefulness:null,novelty:null,repetitionRisk:null,researchCost:0,resourcePressure:null,methodRef:'discovery-prepared-reference:1',limitations:['Unknown scores remain null; source presence never measures personal interest or enjoyment.','Research cost is zero additional acquisition for this prepared candidate; it is not a system performance measurement.','Live selection uses explicit topic matches before lexical overlap; no model-internal influence is claimed.']},confersAuthority:false};
