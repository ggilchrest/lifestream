...
export type DiscoveryLabRow={id:string;split:string;variant:string;prompt:string;note:string;memory:string;manifestDigest:string;sourceRefs:string[];disposition:string;selectedItems:number;tokenUpperBound:number;elapsedMs:number;lookupElapsedMs:number;selectionElapsedMs:number;checks:{name:string;pass:boolean}[]};
