...
import { PwceGatewayClient, PwceCapabilityCatalog, PwceInvalidationStreams, type PwceCapabilityBinding } from '@lifestream/providers-pwce';
...
export class PwceCapabilityDiscovery {
  ...
  async discover(input: PwceCapabilityOwner, inputCall: CapabilityCallContext) {
    ...
    try {
      ...
      const context = { signal: call.context.signal, isCurrent: (scope: CallScope) => isDeepStrictEqual(scope, entry!.scope) && this.current(entry!) && call.context.isCurrent() };
      ...
      const snapshot = result.outcome.payload, bundle = await call.wait(() => this.client.capabilityContracts(call.context.signal)), tools = [];
      const schemaScope = { assistantId: owner.assistantId, endpointId: owner.endpointId, sessionId: owner.sessionId, environment: this.profile.lifestreamEnvironmentId, authorityContextRef: entry.scope.authorityContextRef! };
      for (const definition of snapshot.capabilities) {
        ...
        const inputSchema = await resolveCapabilitySchema(descriptor.inputSchemaArtifact, schemaScope, call, this.catalog.schemas);
        await resolveCapabilitySchema(descriptor.resultSchemaArtifact, schemaScope, call, this.catalog.schemas);
      ...
      call.check();
      if (!this.current(entry) || !this.catalog.retained(snapshot.snapshotId, entry.scope)) return unavailable();
      return { status: 'available', protocol: 'canonical', providerRef: 'pwce', environmentId: this.profile.lifestreamEnvironmentId, expiresAt: new Date(Math.min(Date.parse(snapshot.expiresAt), entry.expiresAt)).toISOString(), tools, grantsAuthority: false, dispatchStarted: false, actionAdministration: 'unavailable', limitation: 'Capability discovery is available; PWCE action preparation and administration are not yet connected.' };
