...
export async function setup(t: { after(callback: () => unknown): void }, configured = true) {
  ...
  const provider: CapabilityProvider = {
    ...
    async invoke(request) { calls++; lastInvocation = structuredClone(request); invoked?.(); if (invokeWait) await invokeWait;
      ...
      return { schemaVersion: '1.0.0', operation: request.operation, requestId: request.requestId, correlationId: request.correlationId, providerRef: 'fixture', completedAt: new Date().toISOString(), outcome: { status: 'succeeded', payload: { type: 'succeeded', invocationId: request.payload.invocationId, output: invokeMode === 'malformed' ? { surprise: true } : { text: (request.payload.input as any).text }, outputSchema: output.artifact, confirmedAt: new Date().toISOString(), evidenceRef }, error: null } };
    }, async getInvocation() { throw new Error('No invocation started'); },
  ...
  const composition = { environmentId: randomUUID(), providerRef: 'fixture', provider, schemas, governance: {
    async evaluate(request: any) {
      const bytes = Buffer.from(JSON.stringify({ invocationId: request.payload.invocationId, inputDigest: request.payload.inputDigest, authority: request.scope.authorityContextRef }));
