...
test('authenticated hypothesis review preserves evidence and rejection survives failed storage and database restore',async t=>{
 ...
 const work={...previous,workId:randomUUID(),idempotencyKey:'synthetic-analysis',purpose:'hypothesisAnalysis',state:'queued',revision:1,admissionReceiptRef:null,lastOutcome:'notRun',producedRefs:[],createdAt:new Date().toISOString()};repo.admit(scope,work,understandingDigest(work),understandingDigest('synthetic-review-seed'),String(saved.boundary),()=>true);repo.start(scope,work.workId);const derivatives=compileDiscoveryCandidates(hypothesis,String(saved.boundary),Date.now());assert.equal(derivatives.some(record=>record.kind==='question'),true);assert.equal(repo.publish(scope,work.workId,String(saved.boundary),[hypothesis,...derivatives],()=>true),true);
 ...
 db.exec('DROP TRIGGER fail_hypothesis_rejection');const rejected=await discovery(reject);assert.equal(rejected.status,200,JSON.stringify(rejected));assert.equal(rejected.body.records[0].status,'rejected');assert.equal(rejected.body.records[0].revision,3);const rejectedInspection=await discovery({operation:'inspect'});assert.equal(rejectedInspection.status,200);assert.ok(rejectedInspection.body.records.filter((record:any)=>record.recordType==='candidate'&&record.hypothesisRefs?.includes(`hypothesis:${hypothesis.hypothesisId}:1`)).every((record:any)=>record.status==='invalidated'),'rejection invalidates dependent candidates');assert.equal((await discovery(reject)).status,200);assert.equal((await discovery({...reject,reason:'changed retry'})).status,409);
