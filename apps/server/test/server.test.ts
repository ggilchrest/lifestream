...
test("relationship administration is authenticated, scoped, reviewable and CAS protected", async (t) => {
  ...
  assert.equal((await fetch(candidatePath, { method: "POST", headers: auth, body: JSON.stringify({ content: "missing revision", source: "synthetic-note", sourceFamily: "fixture", uncertainty: "medium" }) })).status, 409);
  const candidateResponse = await fetch(candidatePath, { method: "POST", headers: auth, body: JSON.stringify({ content: "Prefers concise replies", source: "synthetic-note", sourceFamily: "fixture", uncertainty: "medium", expectedRevision: 1 }) }); assert.equal(candidateResponse.status, 201); const candidate = await candidateResponse.json() as { candidate: { candidateId: string } };
