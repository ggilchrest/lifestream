...
test("fixture Assistant administration persists revisions and requires authority", async (t) => {
  ...
  const transitionResponse = await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories/${memoryBody.memory.id}/lifecycle`, { method: "POST", headers: auth, body: JSON.stringify({ status: "active", reason: "Human reviewed" }) }); assert.equal(transitionResponse.status, 200);
