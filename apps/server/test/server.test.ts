...
test("fixture Assistant administration persists revisions and requires authority", async (t) => {
  ...
  const memoryBody = await memoryResponse.json() as { memory: { assistantId: string; content: string; lifecycle: { status: string }; provenance: { actor: string } } }; assert.equal(memoryBody.memory.assistantId, created.assistantId); assert.equal(memoryBody.memory.lifecycle.status, "candidate"); assert.equal(memoryBody.memory.provenance.actor, "human");
