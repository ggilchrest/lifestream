...
test("fixture Assistant administration persists revisions and requires authority", async (t) => {
  ...
  assert.equal((await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories/${memoryBody.memory.id}/lifecycle`, { method: "POST", headers: auth, body: JSON.stringify({ status: "retracted", expectedRevision: 1 }) })).status, 409);
  assert.equal((await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories/${memoryBody.memory.id}/lifecycle`, { method: "POST", headers: auth, body: JSON.stringify({ status: "retracted", expectedRevision: "1" }) })).status, 422);
  ...
  const listedAdaptations = await (await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/adaptations`, { headers: auth })).json() as { adaptations: { status: string }[] }; assert.deepEqual(listedAdaptations.adaptations.map((adaptation) => adaptation.status), ["active"]);
  ...
  assert.equal((await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories/import`, { method: "POST", headers: auth, body: JSON.stringify({ memories: [{ ...(memoryExport.memories[0] as object), assistantId: "foreign-assistant" }] }) })).status, 422);
  assert.equal((await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories/import`, { method: "POST", headers: auth, body: JSON.stringify({ memories: [{ ...(memoryExport.memories[0] as object), provenance: { grant: "live" } }] }) })).status, 422);
  const importedCandidate = { ...(memoryExport.memories[0] as Record<string, unknown>), id: "imported-memory-id" }; const partialImport = await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories/import`, { method: "POST", headers: auth, body: JSON.stringify({ memories: [importedCandidate, memoryExport.memories[0]] }) }); assert.equal(partialImport.status, 422); const afterPartialImport = await (await fetch(`${base}/api/admin/v1/assistants/${created.assistantId}/memories`, { headers: auth })).json() as { memories: unknown[] }; assert.equal(afterPartialImport.memories.length, 1);
