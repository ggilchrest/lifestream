...
test("relationship configuration reset and rollback create reviewable drafts", async (t) => {
  ...
  const reset = await fetch(`${path}/configurations`, { method: "POST", headers: auth, body: JSON.stringify({ action: "reset", idempotencyKey: "config-reset-1" }) }); assert.equal(reset.status, 201); const resetBody = await reset.json() as { resetTo: string; configuration: { status: string; preset: string } }; assert.equal(resetBody.resetTo, second.configuration.configurationId); assert.equal(resetBody.configuration.status, "draft"); assert.equal(resetBody.configuration.preset, "coaching");
});
...
test("relationship insights keep evidence, communication and salience feedback distinct", async (t) => {
  ...
  const run = await fetch(`${path}/insights`, { method: "POST", headers: auth, body: "{}" }); assert.equal(run.status, 200); const runBody = await run.json() as { insights: { lens: string; supportEvidenceIds: string[]; counterEvidenceIds: string[]; uncertainty: string }[]; activeStateChanged: boolean }; assert.equal(runBody.insights.length, 5); assert.equal(runBody.insights[0].supportEvidenceIds.length, 1); assert.deepEqual(runBody.insights[0].counterEvidenceIds, [secondId]); assert.equal(runBody.insights[0].uncertainty, "insufficient independent evidence"); assert.equal(runBody.activeStateChanged, false);
  const helpful = await fetch(`${path}/feedback`, { method: "POST", headers: auth, body: JSON.stringify({ targetId: firstId, kind: "helpfulness", value: "useful" }) }); assert.equal(helpful.status, 201); assert.equal((await helpful.json() as { feedback: { affects: string; independentSupport: boolean } }).feedback.independentSupport, false);
  const reaffirm = await fetch(`${path}/feedback`, { method: "POST", headers: auth, body: JSON.stringify({ targetId: firstId, kind: "reaffirmation", value: "I explicitly confirm this" }) }); assert.equal(reaffirm.status, 201); assert.equal((await reaffirm.json() as { feedback: { affects: string; independentSupport: boolean } }).feedback.affects, "evidence");
  const listed = await (await fetch(`${path}/insights`, { headers: auth })).json() as { insights: { insightId: string; status: string }[]; feedback: { kind: string }[] }; assert.equal(listed.feedback.length, 2); const reject = await fetch(`${path}/insights/${listed.insights[0].insightId}/decision`, { method: "POST", headers: auth, body: JSON.stringify({ decision: "rejected", expectedStatus: "proposed" }) }); assert.equal(reject.status, 200); assert.equal((await reject.json() as { activeStateChanged: boolean; insight: { status: string } }).activeStateChanged, false);
});
