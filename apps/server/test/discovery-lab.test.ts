...
test('Discovery Lab retains failing expectations and uses bounded canonical input across frozen variants',()=>{
 ...
 const resolved=report.rows.find(row=>row.id==='ambiguous-pair'&&row.variant==='enabled')!;assert.ok(resolved.checks.every(check=>check.pass),'explicit approved favorite now resolves the previously recorded ambiguous input failure');assert.doesNotMatch(resolved.memory,/STELLAR_DETAIL_392/);
