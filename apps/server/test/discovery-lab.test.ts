...
test('Discovery Lab retains failing expectations and uses bounded canonical input across frozen variants',t=>{
 ...
 t.diagnostic(JSON.stringify(report.rows.map(({id,variant,disposition,elapsedMs,lookupElapsedMs,selectionElapsedMs,checks})=>({id,variant,disposition,elapsedMs,lookupElapsedMs,selectionElapsedMs,failed:checks.filter(check=>!check.pass).map(check=>check.name)}))));
 ...
 const resolved=report.rows.find(row=>row.id==='ambiguous-pair'&&row.variant==='enabled')!;assert.ok(resolved.checks.every(check=>check.pass),JSON.stringify(resolved));assert.doesNotMatch(resolved.memory,/STELLAR_DETAIL_392/);
 for(const id of ['current-domain','current-correction','source-detail','heldout-alternate','heldout-mineral'])assert.ok(report.rows.filter(row=>row.id===id).every(row=>row.checks.every(check=>check.pass)),JSON.stringify(report.rows.filter(row=>row.id===id)));
...
test('protected Discovery comparison has no live artifact/configuration writes or provider calls and rejects foreign scope',async t=>{
 ...
 for(let n=0;n<150&&result.status===202;n++){await new Promise(resolve=>setTimeout(resolve,20));result=await discovery(request);}assert.equal(result.status,200,JSON.stringify(result));assert.equal(result.body.activeConfigurationId,null);assert.equal(result.body.activeStateChanged,false);assert.deepEqual(result.body.records,[]);assert.match(result.body.explanations[0].summary,/rows have failed input expectations/);assert.match(result.body.explanations[0].summary,/No model was called/);t.diagnostic(JSON.stringify(result.body.explanations.filter((e:any)=>e.code.endsWith(':enabled:checks'))));const enabledChecks=result.body.explanations.find((e:any)=>e.code==='lab:ambiguous-pair:enabled:checks');assert.ok(enabledChecks&&!enabledChecks.summary.includes('FAIL:'),JSON.stringify(result.body.explanations.filter((e:any)=>e.code.startsWith('lab:ambiguous-pair:enabled:'))));assert.match(enabledChecks.summary,/ms total local selection .*ms lookup\/filter; .*ms inner selection/);assert.deepEqual(snapshot(),before);assert.equal(providerCalls,0);
