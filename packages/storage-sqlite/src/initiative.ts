...
export class InitiativeLedgerRepository {
  ...
  record(opportunity: InitiativeAdmissionRecord, outcome: InitiativeOutcome | null = null, recordedAt = new Date().toISOString()): void {
    this.database.transaction((tx) => { tx.run("INSERT INTO relational_initiative_ledger (opportunity_id, assistant_id, user_id, relationship_id, session_id, endpoint_id, origin, urgency, kind, created_at, expires_at, outcome, recorded_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", opportunity.opportunityId, opportunity.assistantId, opportunity.userId, opportunity.relationshipId, opportunity.sessionId, opportunity.endpointId, opportunity.origin, opportunity.urgency, opportunity.kind, opportunity.createdAt, opportunity.expiresAt, outcome, recordedAt); });
  }
