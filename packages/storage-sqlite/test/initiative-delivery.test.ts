...
test("S083 inference migration upgrades an existing delivery database without rewriting its records or migrations",()=>{
 ...
 try{
  ...
  const upgraded=db.migrate();assert.deepEqual(upgraded.slice(0,oldMigrations.length),oldMigrations);assert.deepEqual(upgraded.slice(oldMigrations.length).map(m=>m.id),[24,25,26,27,28,29]);
