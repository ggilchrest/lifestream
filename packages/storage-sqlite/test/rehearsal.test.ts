...
test("fixture backup and restore preserve migration head and integrity", async () => {
  ...
  const manifest = await backupFixture(source, destinationPath); source.close(); const report = restoreFixture(manifest, (path) => new Database({ path })); assert.equal(report.integrityVerified, true); assert.equal(report.migrationHead, 13); rmSync(directory, { recursive: true, force: true });
});
