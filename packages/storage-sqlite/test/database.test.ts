...
test("SQLite database migrates, persists, and rejects changed history", () => {
  ...
  assert.equal(first.migrate().length, 4); assert.equal(first.migrate().length, 4);
  ...
  const second = new Database({ path }); assert.equal(second.migrate().length, 4); second.close();
