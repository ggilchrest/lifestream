...
test("SQLite database migrates, persists, and rejects changed history", () => {
  ...
  assert.equal(first.migrate().length, 12); assert.equal(first.migrate().length, 12);
  ...
  const second = new Database({ path }); assert.equal(second.migrate().length, 12); second.close();
...
test("sparse migration ledger upgrades without rewriting existing data", () => {
  ...
  assert.equal(records.length, 12); assert.equal(records[0]?.id, migration?.id);
