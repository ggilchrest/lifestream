...
test('explicit no-retry speech admission attempts a busy provider only once without changing the ordinary default',async()=>{
 let calls=0;const provider=new VoxCpmProvider({baseUrl:'http://tts.invalid',voiceBundleKey:'fixture',voiceBundleRevision:1,runtimeRevision:'r',modelRevision:'m',mappingRevision:'map',fetch:async()=>{calls++;return response(JSON.stringify({kind:'terminal',sequence:0,outcome:'providerUnavailable',outputSamples:0,frameCount:0})+'\n');}});
 const events=[];for await(const event of provider.withoutAdmissionRetries().synthesize({...request,deadlineAt:new Date(Date.now()+10000).toISOString()}))events.push(event);
 assert.equal(calls,1);assert.equal(events.length,1);assert.equal(events[0]!.kind,'terminal');
 calls=0;for await(const _event of provider.synthesize({...request,deadlineAt:new Date(Date.now()+10000).toISOString()})){/* Exhaust the unchanged ordinary provider. */}assert.equal(calls,3);
});
